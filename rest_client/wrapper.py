...
class HttpWrapper:
    ...
    async def _listener(self, queue):
    ...
    async def _start_listener(self, bucket, queue):
        ...
        if task is None or task.done():
            ...
            self.listeners[bucket] = asyncio.create_task(self._listener(queue))
        else:
    ...
    async def _request(self, method, endpoint, identifier, future, json=None):
        while True:
            try:
                await self.global_rate_limit.wait()
                self.logger.debug(f"Sending {json}")
    ...
    async def request(self, method, endpoint, identifier, json):
        ...
        if identifier in self.identifiers:
            ...
            await bucket_in_buckets["queue"].put(self._request(method, endpoint, identifier, future, json))

            await self._start_listener(bucket, bucket_in_buckets["queue"])

            return await future
        else:
            await self._request(method, endpoint, identifier, future, json)